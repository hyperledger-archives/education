...
class TunachainTransactionHandler(TransactionHandler):
    ...
    def apply(self, transaction, context):
        ...
        if payload.action == 'create':
            return _create_asset(asset=payload.asset,
                                 owner=signer,
                                 state=state)

        elif payload.action == 'transfer':
            return _transfer_asset(asset=payload.asset,
                                   owner=payload.owner,
                                   signer=signer,
                                   state=state)

        elif payload.action == 'accept':
            return _accept_transfer(asset=payload.asset,
                                    signer=signer,
                                    state=state)

        elif payload.action == 'reject':
            return _reject_transfer(asset=payload.asset,
                                    signer=signer,
                                    state=state)
...
def _create_asset(asset, owner, state):
    ...
    return state.set_asset(asset, owner)
...
def _transfer_asset(asset, owner, signer, state):
    ...
    return state.set_transfer(asset, owner)
...
def _accept_transfer(asset, signer, state):
    ...
    return state.delete_transfer(asset)
